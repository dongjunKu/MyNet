...
class GaussKernel2dFn(Function):
    ...
    def forward(ctx, input, kernel_size, stride, padding, dilation, channel_wise):
        ...
        if not channel_wise:
            diff_sq = diff_sq.sum(dim=1, keepdim=True) # 채널 축에 대한 노름으로.
        output = torch.exp(-0.5 * diff_sq) # 차이가 작으면 1, 차이가 크면 0에 가까운 마스크
        ctx._backend = type2backend[input.type()]
        ctx.save_for_backward(input, output)

        return output # 여전히 unfold 상태임에 유의. 이거를 unfold된 input과 곱하면 각 위치마다 위치에 적응된 마스크를 씌우는 효과를 냄. 이 곳이 핵심!!
...
# 입력을 unfold한 다음 이미 unfold된 커널을 받아서 hadamard 곱함.
class PacConv2dFn(Function):
    ...
    def forward(ctx, input, kernel, weight, bias=None, stride=1, padding=0, dilation=1, shared_filters=False):
        ...
        in_mul_k = cols.view(bs, ch, *kernel.shape[2:]) * kernel # 유사도 가우시안 필터 적용, hadamard 곱

        # https://pytorch.org/docs/stable/generated/torch.einsum.html
        # 아마 이 부분이 fold 작업을 포함하고 있는거라고 예상.
        ...
        if shared_filters:
            ...
            output = torch.einsum('ijklmn,ojkl->iomn', (in_mul_k, weight)) # weight 적용
...
def packernel2d(input, mask=None, kernel_size=0, stride=1, padding=0, output_padding=0, dilation=1,
    ...
    if native_impl: # native_impl와 else의 코드 내용 차이는 그냥 클래스를 썼느냐 아니냐의 차이인 듯? 사실상 else의 GaussKernel2dFn만 보면 됨.
